...
The helper functions above make it easy to provide routine operations for multiple coins. The next code section below loads actual crypto data, using the wrapper above when necessary. To start, we load roughly the last year of daily data for all the top 25 coins. Note that some of the coins in this list have not even been around for a year, so we'll start our analysis on a few we know will have sufficient data. Those are BTC (Bitcoin), Ethereum (ETH), and XMR (Monero).
...
We start off analysis by looking at qualitative and quantitative tests for the coins we've selected. We always start with the raw data itself. In general, we are interested in basic characteristics of our time series. 
